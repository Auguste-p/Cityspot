...
| ID | Type | Criticité | Scénario | Étapes | Résultat attendu | Statut |
|---|---|---|---|---|---|---|
| MUN-01 | F | Majeur | Accès via un compte municipal | Se connecter avec `role = municipal` | Bouton d'accès visible dans l'en-tête, page accessible | ☐ *(non exécuté : nécessite un compte avec `user_metadata.role = 'municipal'`, non disponible parmi les comptes de test au moment de la rédaction)* |
...
| MUN-03 | F | Mineur | Filtrage par catégorie | Cliquer sur chaque catégorie (voirie, éclairage, sécurité, propreté, espaces verts, mobilier urbain) | Liste filtrée, compteur par catégorie exact | ☐ *(bloqué par MUN-01, même cause)* |
| MUN-04 | F | Mineur | Onglets par statut | Parcourir les onglets Tous/En vote/En cours/Terminés | Contenu et compteurs cohérents avec les données | ☐ *(bloqué par MUN-01, même cause)* |
| MUN-05 | F | Mineur | Statistiques globales | Comparer les cartes de stats en haut de page aux données réelles | Total, en vote, en cours, terminés corrects | ☐ *(bloqué par MUN-01, même cause)* |
| MUN-06 | F | Mineur | État vide | Filtrer une catégorie sans signalement | Message "Aucun signalement dans cette catégorie" | ☐ *(bloqué par MUN-01, même cause)* |
...
| PROF-07 | F | Mineur | Badge "Mairie" conditionnel | Ouvrir le profil d'un compte `cityWorker = true`, puis d'un compte `cityWorker = false` | Badge dégradé bleu "Mairie" (icône bâtiment) visible uniquement dans le premier cas | ☐ *(non re-testé cette session pour limiter les manipulations sur les comptes de test ; vérifié visuellement lors du développement de la fonctionnalité)* |
...
| SEC-05 | SEC | Majeur | Double vote | Voter deux fois avec le même compte sur le même post | Second vote bloqué côté UI (`hasVoted`) et rejeté côté base | ❌ *(exécuté 2026-07-17 par INSERT REST direct — deux votes identiques (même `id_user`, même `id_issue`) tous deux acceptés en HTTP 201. **Confirmé : aucune contrainte d'unicité sur la table `votes`.** Le blocage `hasVoted` n'est que visuel ; un doublon en base est possible en contournant l'UI)* |
...
| SEC-10 | SEC | Majeur | Isolation des données par ligne (RLS) — `issues`/`tasks`/`materials`/`comments`/`votes` | Depuis un compte citoyen, tenter une requête Supabase directe (hors UI) sur ces tables pour un autre utilisateur | Row Level Security doit empêcher toute lecture/écriture non autorisée | ❌ *(exécuté 2026-07-17 par PATCH REST direct sur un signalement, en contournant complètement `updateIssue` et sa vérification `created_by` — HTTP 200, modification acceptée. **Confirmé : `issues` n'a aucune RLS bloquant une modification arbitraire au niveau base.** Cohérent avec DET-15 : la seule protection sur l'édition est côté client)* |
...
|---|---|
| ✅ OK | 66 |
| ❌ KO confirmé | 2 (SEC-05, SEC-10) |
| ☐ Non exécuté (raison documentée par ligne) | 19 |
| **Total** | **87** |
...
- Les scénarios **Majeur** en ❌ doivent soit être corrigés, soit faire l'objet d'un correctif planifié et documenté dans le plan de correction des bogues (C2.3.2). **2 scénarios Majeur sont actuellement ❌ (SEC-05, SEC-10) — non bloquants pour la mise en production au sens strict du seuil ci-dessus, mais à traiter avant une recette finale complète.**
...
- **Nouveau, confirmé par exécution le 2026-07-17 — SEC-05** : la table `votes` n'a aucune contrainte d'unicité sur (`id_user`, `id_issue`). Deux votes identiques du même compte sur le même signalement sont acceptés par un INSERT REST direct (contournant l'UI). Le blocage `hasVoted` dans `PostDetail.tsx`/`MapView.tsx` n'est que visuel — un utilisateur technique peut voter plusieurs fois et fausser le tally.
- **Nouveau, confirmé par exécution le 2026-07-17 — SEC-10** : `issues` n'a aucune RLS empêchant une modification arbitraire. Un PATCH REST direct sur un signalement quelconque (contournant `updateIssue` et sa vérification `created_by`) est accepté sans erreur — n'importe quel compte authentifié peut modifier n'importe quel signalement au niveau base, indépendamment du garde côté client mis en place pour DET-15.
...
- Le projet Supabase applique un rate limit par défaut de 2 signups/heure. Deux comptes de test fournis manuellement (dont un nécessitant une confirmation d'email forcée en base, `auth.users.email_confirmed_at`, l'auto-confirmation du `config.toml` local ne s'appliquant pas au projet distant) ont permis d'exécuter DET-09, SEC-02, SEC-11 et PROF-08. AUTH-01 a été validé séparément par une inscription manuelle réussie sur le site.
- MUN-01/03/04/05/06 restent non exécutés faute d'un compte avec `user_metadata.role = 'municipal'` parmi les comptes de test disponibles au moment de la rédaction.
- SEC-01 reste non exécutable par l'agent de test : le scénario nécessite d'éditer temporairement le `.env` réel du projet (clé commençant par `sb_secret_`), ce que la politique de permissions de l'environnement d'exécution a explicitement refusé (lecture/écriture de `.env` bloquée) — à exécuter manuellement.
- STR-04 (pipeline CI) volontairement non déclenché de façon synthétique : mieux vérifié lors d'un vrai push que par une action artificielle sur le dépôt.
