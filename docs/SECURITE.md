...
| **A06:2021 – Vulnerable and Outdated Components** | `npm audit --omit=dev --audit-level=high` rejoué à chaque push/PR vers `main` — le build échoue si une vulnérabilité haute/critique touche une dépendance de production ; une mise à jour ciblée de Vite avait déjà résolu 3 vulnérabilités hautes en amont, `postcss` et `react-router` corrigés le 2026-07-29 (`npm audit fix`) | `.github/workflows/ci.yml` ; procédure manuelle documentée dans `MANUEL_MISE_A_JOUR.md` §6.2 ; détail du scope `--omit=dev` : `MAINTENANCE.md` §2 et §8 | CI (`ci.yml`) à chaque push/PR |
...
| **A08:2021 – Software and Data Integrity Failures** | CI (`typecheck` + `lint` + `test:coverage` + `build`) bloque la fusion sur `main` en cas d'échec ; `package-lock.json` committé + `npm ci` (CI et `Dockerfile`) garantissent des builds reproductibles à partir des mêmes versions verrouillées ; toute évolution de schéma passe par une migration versionnée relue avant application | `.github/workflows/ci.yml`, `Dockerfile` | — |
...
Aucune limite résiduelle sur A08 : `package-lock.json` est committé depuis le 2026-07-20 (cf. `MANUEL_DEPLOIEMENT.md` §10), les builds CI et Docker sont reproductibles via `npm ci`.
