...
| MAP-04 | F | Mineur | Recentrage sur la ville du profil | Cliquer sur "Me localiser" | Carte centrée sur la ville renseignée au profil (`cityLat`/`cityLng`), nom de ville affiché | ☐ *(comportement changé le 2026-09-02 : le bouton ne géolocalise plus l'appareil, il recentre directement sur la ville du profil — recette à rejouer)* |
| MAP-05 | F | Mineur | Repli sans ville renseignée | Cliquer sur "Me localiser" avec un profil sans ville | Carte centrée sur la ville de repli (`FALLBACK_CITY`), pas de blocage | ☐ *(comportement changé le 2026-09-02, idem MAP-04 — recette à rejouer)* |
...
| SEC-02 | SEC | Bloquant | Suppression par un non-propriétaire | `DELETE` REST direct sur `issues` (`.eq('id', ...).select('id')`) avec le token d'un utilisateur différent du créateur | `HTTP 200`, tableau de lignes supprimées vide (RLS filtre silencieusement) → `deleteIssue()` lève "Vous n'êtes pas autorisé à supprimer ce signalement" | ✅ *(re-exécuté 2026-07-19 par sonde REST directe avec 2 comptes réels — signalement créé par le compte A, tentative de suppression par le compte B : `HTTP 200`, `[]`. Signalement toujours présent après coup, confirmé par relecture. Nouveau mécanisme depuis le retrait de l'Edge Function `delete-issue`, cf. `CHANGELOG.md` v1.2.0)* |
| SEC-03 | SEC | Bloquant | Suppression sans authentification | Même appel, en anonyme (sans session) | Même résultat que SEC-02 : `HTTP 200`, 0 ligne supprimée (RLS traite l'anonyme comme un non-propriétaire, pas de distinction) | ✅ *(re-exécuté 2026-07-19 par sonde REST directe — `Authorization` réduit à la clé anon (pas de session) : `HTTP 200`, `[]`, même résultat que SEC-02. Suppression légitime par le compte propriétaire vérifiée dans la foulée pour confirmer que le mécanisme fonctionne bien pour le cas nominal (`HTTP 200`, ligne renvoyée))* |
